...
export class CompositeParser extends DelegateParser {
    ...
    _complete(): void {
        ...
        for (var name in this._undefined) {
            if (this._defined[name] === undefined) {
                throw new UndefinedProductionError(new Error(), name);
            }
    ...
    def(name: string, parser: Parser): void {
        if (this._completed) {
            throw new CompletedParserError(new Error());
        } else if (this._defined[name] !== undefined) {
            throw new RedefinedProductionError(new Error(), name);
        } else {
...
// Error types have moved from here into "errors.ts"
