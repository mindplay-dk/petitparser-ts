...
function expect(value: any, match: any) {
    if (match instanceof Matcher) {
        ...
        ok(compareArrays(value, match), 'arrays are equal');
    } else {
        equal(value, match);
    }
}
...
function group(name: string, func: () => void) {
    QUnit.module(name);
    func();
}
...
class PluggableCompositeParser extends CompositeParser {
    ...
    /** @override */
    initialize(): void { this._func(this) }
}
...
group('parsers', () => {
    ...
    test('map()', () => {
        var parser = digit().map((each: string) => {
    ...
    test('separatedBy() without separators', () => {
        var parser = char('a').separatedBy(char('b'), false);
        expectFailure(parser, '', 0, 'a expected');
        expectSuccess(parser, 'a', ['a']);
        expectSuccess(parser, 'ab', ['a'], 1);
    ...
    test('separatedBy() separator at end', () => {
        var parser = char('a').separatedBy(char('b'), true, false);
        expectFailure(parser, '', 0, 'a expected');
        expectSuccess(parser, 'a', ['a']);
    ...
    test('separatedBy() without separators & separator at end', () => {
        var parser = char('a').separatedBy(char('b'), false, true);
        expectFailure(parser, '', 0, 'a expected');
        expectSuccess(parser, 'a', ['a']);
